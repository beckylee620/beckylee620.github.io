...
$( "#stage" ).draggable({
  axis:"x",
  drag:function() {
    var movedLeft = Math.round(parseInt($(this).css('left')));
    var criticalDistanceRight = Math.round(parseInt($(this).css('width')) / 11);
    var criticalDistanceLeft = Math.round(parseInt($(this).css('width')) / -11); //Math.round(parseInt($(this).css('width')));
    if(movedLeft > criticalDistanceRight && movedLeft % criticalDistanceRight == 0) {
      countUp();
      buildBlocks(zero);
      movedLeft = 0;
    } else if(movedLeft < criticalDistanceLeft && movedLeft % criticalDistanceLeft == 0) {
      countDown();
      buildBlocks(zero);
      movedLeft = 0;
    }
